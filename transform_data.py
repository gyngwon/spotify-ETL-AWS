...
def lambda_handler(event, context):
    ...
    Key = "discover_daily/raw_data/to_process/"
    
    spotify_data = []
    spotify_keys = []
    
    # List objects that take in the bucket name and prefix which is the key
    ...
    for data in spotify_data:
        ...
        songs_key = "discover_daily/transformed_data/song_data/songs_transformed_" + str(datetime.now().strftime('%Y-%m-%d_%H-%M-%S')) + ".csv"
        song_buffer = StringIO()
        song_df.to_csv(song_buffer, index=False)
        song_content = song_buffer.getvalue()
        s3.put_object(Bucket=Bucket, Key=songs_key, Body=song_content)
        
        album_key = "discover_daily/transformed_data/album_data/album_transformed_" + str(datetime.now().strftime('%Y-%m-%d_%H-%M-%S')) + ".csv"
        album_buffer = StringIO()
        album_df.to_csv(album_buffer, index=False)
        album_content = album_buffer.getvalue()
        s3.put_object(Bucket=Bucket, Key=album_key, Body=album_content)
        
        artist_key = "discover_daily/transformed_data/artist_data/artist_transformed_" + str(datetime.now().strftime('%Y-%m-%d_%H-%M-%S')) + ".csv"
    ...
    for key in spotify_keys:
        ...
        s3_resource.meta.client.copy(copy_source, Bucket, 'discover_daily/raw_data/processed/' + key.split("/")[-1])    
